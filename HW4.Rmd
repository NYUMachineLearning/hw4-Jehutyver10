...
highly_correlated
```
Using Pearson Correlation for filter-based feature selection, the variables chas, ptratio, rm, medv, and rad are all highly correlated.
...
# run the RFE algorithm
results = rfe(BostonHousing_num[,1:13], BostonHousing_num[,14], sizes = c(2,5,9), rfeControl = control, method = "svmRadial")

results
results$variables

```
Using the Recursive Feature Elimination method, lstat, rm, ptratio, indus, and crim are the most highly correlated variables. Of these, only rm and ptratio are common to both this method and the Pearson correlation method.
...
boruta <- Boruta(Class ~ ., data=na.omit(BreastCancer_num), doTrace=0)  
names(boruta)
signifs <- getSelectedAttributes(boruta, withTentative = TRUE)
roughFix <- TentativeRoughFix(boruta)
signifs <- getSelectedAttributes(roughFix)
print(signifs)
imps <- attStats(roughFix)
impsConfirmed = imps[imps$decision != 'Rejected', c('meanImp', 'decision')]
head(impsConfirmed[order(-impsConfirmed$meanImp), ])  # descending sort
plot(boruta, cex.axis=.7, las=2, xlab="", main="Variable Importance")  



```
Mitoses, Epith.c.size, Marg.adhesion, Normal.nucleoli, Bl.cromatin, Cl.thickness, Cell.Shape, Cell.size, and Bare.nuclei are confirmed important variables using Boruta.